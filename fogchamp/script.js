...
visualizer_UI.prototype = {
	...
	,fetchMovesetsFromAPI: function() {
		...
		var showProgressCallback = function(value) {
			_gthis.userMessage.showMessage("Loading Movesets from TPP. Progress: " + value);
		};
	...
	,renderPokemonStats: function() {
		var template = $("#pokemonStatsTemplate").html();
		var rendered = visualizer_UI.renderTemplate(template,{ pokemonStats : this.buildPokemonStatsRenderDocs(true)});
		$("#pokemonStats").html(rendered);
	}
	,buildPokemonStatsRenderDocs: function(visualBlueHorizontalOrder) {
		...
		while(_g < slotNums.length) {
			...
			if(__map_reserved[key] != null ? _this.existsReserved(key) : _this.h.hasOwnProperty(key)) {
				abilityName = this.database.descriptionsDataset.getAbilityName(pokemonStats.ability);
			}
			var itemName = "";
			...
			if(__map_reserved[key1] != null ? _this1.existsReserved(key1) : _this1.h.hasOwnProperty(key1)) {
				itemName = this.database.descriptionsDataset.getItemName(pokemonStats.item);
			}
	...
	,renderPokemonMoves: function() {
		var template = $("#pokemonMovesTemplate").html();
		var rendered = visualizer_UI.renderTemplate(template,{ pokemonMoves : this.buildMovesRenderDocs()});
		$("#pokemonMoves").html(rendered);
	}
	,buildMovesRenderDocs: function() {
		...
		while(_g < _g1.length) {
			var slotNum = _g1[_g];
			++_g;
			var pokemonStat = this.currentPokemon[slotNum];
			...
			while(_g2 < moveSlugs.length) {
				...
				var damageCategory = moveStats.damageCategory;
				moveRenderDoc["damage_category_short"] = HxOverrides.substr(damageCategory,0,2);
	...
	,buildEditAbilityRenderDoc: function(pokemonStats) {
		...
		var abilitySlug = this.database.descriptionsDataset.abilities.keys();
	...
	,buildEditItemRenderDoc: function(pokemonStats) {
		...
		var itemSlug = this.database.descriptionsDataset.items.keys();
	...
	,buildEditMoveRenderDoc: function(pokemonStats,slot) {
		...
		var moveSlug = this.database.movesDataset.moves.keys();
	...
	,attachEditFormListeners: function(pokemonStats,slotNum) {
		...
		var readValues = function(event) {
			...
			var readValues1 = hpInput.val();
			pokemonStats.hp = Std.parseInt(readValues1);
			var readValues2 = attackInput.val();
			pokemonStats.attack = Std.parseInt(readValues2);
			var readValues3 = defenseInput.val();
			pokemonStats.defense = Std.parseInt(readValues3);
			var readValues4 = specialAttackInput.val();
			pokemonStats.specialAttack = Std.parseInt(readValues4);
			var readValues5 = specialDefenseInput.val();
			pokemonStats.specialDefense = Std.parseInt(readValues5);
			var readValues6 = speedInput.val();
			pokemonStats.speed = Std.parseInt(readValues6);
	...
	,renderChart: function() {
		...
		var array = this.currentPokemon.slice(0);
...
visualizer_api_APIFacade.prototype = {
	...
	,parseMovesetPokemonStats: function(jsonDoc,stats) {
		...
		var _g1 = 0;
		while(_g1 < itemsDoc.length) {
			var doc1 = itemsDoc[_g1];
			++_g1;
		...
		var _g2 = 0;
		while(_g2 < movesDoc.length) {
			var slotDoc = movesDoc[_g2];
			++_g2;
			var slotMoves = [];
			var _g11 = 0;
...
visualizer_dataset_PokemonDataset.prototype = $extend(visualizer_dataset_Dataset.prototype,{
	...
	,getPokemonStats: function(slug) {
		...
		pokemonStat.fromJsonObject(Reflect.field(this.get_stats(),slug));
		return pokemonStat;
	}
	,getSlug: function(pokemonNum) {
		var _g = 0;
