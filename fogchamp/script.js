...
visualizer_Main.prototype = {
	...
	,loadAPIMovesets: function() {
		try {
			...
			if(!((haxe_Exception.caught(_g).unwrap()) instanceof visualizer_dataset_StorageEmpty)) {
				throw _g;
			}
		}
		this.loadUI();
...
var visualizer_Orientation = $hxEnums["visualizer.Orientation"] = { __ename__:true,__constructs__:null
	,Vertical: {_hx_name:"Vertical",_hx_index:0,__enum__:"visualizer.Orientation",toString:$estr}
	,Horizontal: {_hx_name:"Horizontal",_hx_index:1,__enum__:"visualizer.Orientation",toString:$estr}
};
visualizer_Orientation.__constructs__ = [visualizer_Orientation.Vertical,visualizer_Orientation.Horizontal];
...
visualizer_MatchupChart.prototype = {
	...
	,renderTable: function() {
		...
		var _g = 0;
		var _g1 = visualizer_MatchupChart.NUM_POKEMON_PER_TEAM * (visualizer_MatchupChart.NUM_MOVES_PER_POKEMON + visualizer_MatchupChart.DIVIDER);
		while(_g < _g1) {
			var moveRowIndex = _g++;
	...
	,renderTopPokemonMovesRow: function(rowElement) {
		...
		var _g = 0;
		var _g1 = visualizer_MatchupChart.NUM_MOVES_PER_POKEMON;
		while(_g < _g1) {
			var moveIndex = _g++;
	...
	,renderMoveRow: function(rowIndex,rowElement) {
		...
		var topPokemonStat = this.pokemonStatsList[3];
		this.renderVersusMatrix(rowElement,leftMoveIndex,leftPokemonStat,topPokemonStat);
		var topPokemonStat = this.pokemonStatsList[4];
		this.renderVersusMatrix(rowElement,leftMoveIndex,leftPokemonStat,topPokemonStat);
		var topPokemonStat = this.pokemonStatsList[5];
		this.renderVersusMatrix(rowElement,leftMoveIndex,leftPokemonStat,topPokemonStat);
	}
	,renderVersusMatrix: function(rowElement,leftMoveIndex,leftPokemonStat,topPokemonStat) {
		if(leftMoveIndex == -1) {
			...
			if(leftMoveIndex < leftPokemonMoveSlugs.length) {
				var moveStat = this.database.movesDataset.getMoveStats(leftPokemonMoveSlugs[leftMoveIndex],leftPokemonStat);
				this.processCellEfficacy(cell,moveStat,leftPokemonStat,topPokemonStat,"left");
			} else {
				this.processCellEfficacy(cell,null,leftPokemonStat,topPokemonStat,"left");
			}
			if(leftMoveIndex == 3) {
...
visualizer_UI.prototype = {
	...
	,renderSelectionList: function() {
		...
		var rendered = visualizer_UI.renderTemplate(template,{ selections : selections, slots : [3,4,5]});
		$("#pokemonSelectionRed").html(rendered);
	}
	...
	,attachSelectChangeListeners: function() {
		var _gthis = this;
		var jquery = $("#selectionSelect" + 0);
		jquery.change(function(event) {
			_gthis.selectChangedCallback(0,jquery.val());
		});
		jquery.focus(function(event) {
			$(".pokemonIconSlot-" + 0).addClass("pokemonIcon-focus");
		});
		jquery.focusout(function(event) {
			$(".pokemonIconSlot-" + 0).removeClass("pokemonIcon-focus");
		});
		var jquery1 = $("#selectionSelect" + 1);
		jquery1.change(function(event) {
			_gthis.selectChangedCallback(1,jquery1.val());
		});
		jquery1.focus(function(event) {
			$(".pokemonIconSlot-" + 1).addClass("pokemonIcon-focus");
		});
		jquery1.focusout(function(event) {
			$(".pokemonIconSlot-" + 1).removeClass("pokemonIcon-focus");
		});
		var jquery2 = $("#selectionSelect" + 2);
		jquery2.change(function(event) {
			_gthis.selectChangedCallback(2,jquery2.val());
		});
		jquery2.focus(function(event) {
			$(".pokemonIconSlot-" + 2).addClass("pokemonIcon-focus");
		});
		jquery2.focusout(function(event) {
			$(".pokemonIconSlot-" + 2).removeClass("pokemonIcon-focus");
		});
		var jquery3 = $("#selectionSelect" + 3);
		jquery3.change(function(event) {
			_gthis.selectChangedCallback(3,jquery3.val());
		});
		jquery3.focus(function(event) {
			$(".pokemonIconSlot-" + 3).addClass("pokemonIcon-focus");
		});
		jquery3.focusout(function(event) {
			$(".pokemonIconSlot-" + 3).removeClass("pokemonIcon-focus");
		});
		var jquery4 = $("#selectionSelect" + 4);
		jquery4.change(function(event) {
			_gthis.selectChangedCallback(4,jquery4.val());
		});
		jquery4.focus(function(event) {
			$(".pokemonIconSlot-" + 4).addClass("pokemonIcon-focus");
		});
		jquery4.focusout(function(event) {
			$(".pokemonIconSlot-" + 4).removeClass("pokemonIcon-focus");
		});
		var jquery5 = $("#selectionSelect" + 5);
		jquery5.change(function(event) {
			_gthis.selectChangedCallback(5,jquery5.val());
		});
		jquery5.focus(function(event) {
			$(".pokemonIconSlot-" + 5).addClass("pokemonIcon-focus");
		});
		jquery5.focusout(function(event) {
			$(".pokemonIconSlot-" + 5).removeClass("pokemonIcon-focus");
		});
	}
	...
	,attachEditionSelectListener: function() {
		...
		$("#pokemonEditionSelect").change(function(event) {
			_gthis.selectEdition($(selectElement).val());
		});
	}
	...
	,readUrlFragment: function() {
		var fragment = $global.location.hash;
		...
		if(pattern.match(fragment)) {
			...
			var this1 = new Array(6);
			var movesetNames = this1;
			pokemonNums[0] = Std.parseInt(pattern.matched(1));
			movesetNames[0] = StringTools.replace(pattern.matched(2),"_"," ");
			pokemonNums[1] = Std.parseInt(pattern.matched(3));
			movesetNames[1] = StringTools.replace(pattern.matched(4),"_"," ");
			pokemonNums[2] = Std.parseInt(pattern.matched(5));
			movesetNames[2] = StringTools.replace(pattern.matched(6),"_"," ");
			pokemonNums[3] = Std.parseInt(pattern.matched(7));
			movesetNames[3] = StringTools.replace(pattern.matched(8),"_"," ");
			pokemonNums[4] = Std.parseInt(pattern.matched(9));
			movesetNames[4] = StringTools.replace(pattern.matched(10),"_"," ");
			pokemonNums[5] = Std.parseInt(pattern.matched(11));
			movesetNames[5] = StringTools.replace(pattern.matched(12),"_"," ");
	...
	,writeUrlFragment: function() {
		var fragment_b = "";
		fragment_b += "#";
		var pokemonStats = this.currentPokemon[0];
		...
		fragment_b += "-";
		var pokemonStats = this.currentPokemon[1];
		var pokemonNum = pokemonStats.number;
		fragment_b += Std.string("" + pokemonNum);
		if(pokemonStats.movesetName != null) {
			fragment_b += Std.string(StringTools.replace(visualizer_api_APIFacade.slugify(pokemonStats.movesetName),"-","_"));
		...
		fragment_b += "-";
		var pokemonStats = this.currentPokemon[2];
		var pokemonNum = pokemonStats.number;
		fragment_b += Std.string("" + pokemonNum);
		if(pokemonStats.movesetName != null) {
			fragment_b += Std.string(StringTools.replace(visualizer_api_APIFacade.slugify(pokemonStats.movesetName),"-","_"));
		...
		fragment_b += "-";
		var pokemonStats = this.currentPokemon[3];
		var pokemonNum = pokemonStats.number;
		fragment_b += Std.string("" + pokemonNum);
		if(pokemonStats.movesetName != null) {
			fragment_b += Std.string(StringTools.replace(visualizer_api_APIFacade.slugify(pokemonStats.movesetName),"-","_"));
		...
		fragment_b += "-";
		var pokemonStats = this.currentPokemon[4];
		var pokemonNum = pokemonStats.number;
		fragment_b += Std.string("" + pokemonNum);
		if(pokemonStats.movesetName != null) {
			fragment_b += Std.string(StringTools.replace(visualizer_api_APIFacade.slugify(pokemonStats.movesetName),"-","_"));
		...
		fragment_b += "-";
		var pokemonStats = this.currentPokemon[5];
		var pokemonNum = pokemonStats.number;
		fragment_b += Std.string("" + pokemonNum);
		if(pokemonStats.movesetName != null) {
			fragment_b += Std.string(StringTools.replace(visualizer_api_APIFacade.slugify(pokemonStats.movesetName),"-","_"));
		...
		$global.location.hash = fragment_b;
	}
	...
	,fetchFromAPI: function() {
		...
		this.database.currentMatchDataset.load(function(loadEvent) {
			...
			if(loadEvent.success) {
				_gthis.setSelectionByAPI(_gthis.database.getCurrentMatchPokemonStats());
	...
	,fetchMovesetsFromAPI: function() {
		...
		this.database.apiPokemonDataset.load(function(event) {
			if(event.success) {
				...
				_gthis.selectEdition(_gthis.database.getEditionNames()[_gthis.database.getEditionNames().length - 1]);
			} else {
	...
	,setSelectionByNumbers: function(pokemonNums,movesetNames) {
		...
		while(_g < 6) {
			...
			try {
				...
				if(((haxe_Exception.caught(_g1).unwrap()) instanceof visualizer_model_StatsNotFoundError)) {
			...
			try {
				...
				if(((haxe_Exception.caught(_g1).unwrap()) instanceof visualizer_model_StatsNotFoundError)) {
					try {
						...
						if(((haxe_Exception.caught(_g2).unwrap()) instanceof visualizer_model_StatsNotFoundError)) {
							this.userMessage.showMessage("The dataset for this edition is corrupt, unrecognized, or not yet loaded.");
							throw haxe_Exception.thrown("Missing dataset");
						} else {
							throw _g2;
						}
					}
	...
	,setSelectionByAPI: function(pokemonStatsList) {
		...
		pokemonStats.update(pokemonStatsList[0]);
		this.currentPokemon[0] = pokemonStats;
		var pokemonStats = new visualizer_datastruct_VisualizerPokemonStats();
		pokemonStats.update(pokemonStatsList[1]);
		this.currentPokemon[1] = pokemonStats;
		var pokemonStats = new visualizer_datastruct_VisualizerPokemonStats();
		pokemonStats.update(pokemonStatsList[2]);
		this.currentPokemon[2] = pokemonStats;
		var pokemonStats = new visualizer_datastruct_VisualizerPokemonStats();
		pokemonStats.update(pokemonStatsList[3]);
		this.currentPokemon[3] = pokemonStats;
		var pokemonStats = new visualizer_datastruct_VisualizerPokemonStats();
		pokemonStats.update(pokemonStatsList[4]);
		this.currentPokemon[4] = pokemonStats;
		var pokemonStats = new visualizer_datastruct_VisualizerPokemonStats();
		pokemonStats.update(pokemonStatsList[5]);
		this.currentPokemon[5] = pokemonStats;
		this.reloadSelectionList();
		this.syncSelectionListToCurrent();
		this.renderAll();
	}
	...
	,syncSelectionListToCurrent: function() {
		var stats = this.currentPokemon[0];
		$("#selectionSelect" + 0).prop("disabled","disabled").val(stats.slug).prop("disabled",false);
		var stats = this.currentPokemon[1];
		$("#selectionSelect" + 1).prop("disabled","disabled").val(stats.slug).prop("disabled",false);
		var stats = this.currentPokemon[2];
		$("#selectionSelect" + 2).prop("disabled","disabled").val(stats.slug).prop("disabled",false);
		var stats = this.currentPokemon[3];
		$("#selectionSelect" + 3).prop("disabled","disabled").val(stats.slug).prop("disabled",false);
		var stats = this.currentPokemon[4];
		$("#selectionSelect" + 4).prop("disabled","disabled").val(stats.slug).prop("disabled",false);
		var stats = this.currentPokemon[5];
		$("#selectionSelect" + 5).prop("disabled","disabled").val(stats.slug).prop("disabled",false);
	}
	...
	,getMatchNumbers: function() {
		var numbers = [];
		var pokemonNum = this.currentPokemon[0].number;
		numbers.push(pokemonNum);
		var pokemonNum = this.currentPokemon[1].number;
		numbers.push(pokemonNum);
		var pokemonNum = this.currentPokemon[2].number;
		numbers.push(pokemonNum);
		var pokemonNum = this.currentPokemon[3].number;
		numbers.push(pokemonNum);
		var pokemonNum = this.currentPokemon[4].number;
		numbers.push(pokemonNum);
		var pokemonNum = this.currentPokemon[5].number;
		numbers.push(pokemonNum);
		return numbers;
	}
	,attachHelpListeners: function() {
		var _gthis = this;
		var _g_i = 0;
		var _g_j = $("[data-help-slug]");
		while(_g_i < _g_j.length) {
			var element = [$(_g_j[_g_i++])];
			var clickElement = $("<a href=>");
			clickElement.addClass("clickHelp");
			clickElement.click((function(element) {
				return function(event) {
					_gthis.clickedHelp(element[0].attr("data-help-slug"));
					return false;
				};
			})(element));
	...
	,clickedHelp: function(helpSlug) {
		...
		if(category == "ability") {
			...
			var template = $("#itemDescriptionTemplate").html();
			var item = this.database.descriptionsDataset.getItem(slug);
			title = item.name;
			html = visualizer_UI.renderTemplate(template,{ "simple" : item.description, "short" : item.effectShort, "long" : item.effectLong});
		} else if(category == "move") {
			var template = $("#moveDescriptionTemplate").html();
			var move = this.database.movesDataset.getMoveStats(slug);
			title = move.name;
			html = visualizer_UI.renderTemplate(template,{ "simple" : move.description, "short" : StringTools.replace(move.effectShort,"$effect_chance%","" + move.effectChance + "%"), "long" : StringTools.replace(move.effectLong,"$effect_chance%","" + move.effectChance + "%"), "flags" : move.flags != null ? "🗹 " + move.flags.join(", 🗹 ") : "", "note" : move.editorNote});
		} else if(category == "damage") {
			var template = $("#moveDamageTemplate").html();
			title = "" + parts[1] + " Damage";
			html = visualizer_UI.renderTemplate(template,{ min_percent : parts[2], max_percent : parts[3], crit_percent : parts[4], min_points : parts[5], max_points : parts[6], crit_points : parts[7]});
		}
	...
	,attachEditListeners: function() {
		...
		while(_g_i < _g_j.length) {
			...
			clickElement.click((function(element) {
				return function(event) {
					_gthis.clickedEdit(Std.parseInt(element[0].attr("data-edit-slot")));
					return false;
				};
			})(element));
			element[0].wrapInner(clickElement);
	...
	,buildEditAbilityRenderDoc: function(pokemonStats) {
		...
		if(pokemonStats.abilitySet != null) {
			setList = new haxe_iterators_ArrayIterator(pokemonStats.abilitySet);
		} else {
			setList = new haxe_ds__$StringMap_StringMapKeyIterator(this.database.descriptionsDataset.abilities.h);
		}
		var abilitySlug = setList;
		while(abilitySlug.hasNext()) {
			var abilitySlug1 = abilitySlug.next();
			abilityRenderList.push({ "slug" : abilitySlug1, "label" : this.database.descriptionsDataset.getAbilityName(abilitySlug1), "selected" : abilitySlug1 == pokemonStats.ability ? "selected" : ""});
		}
	...
	,buildEditItemRenderDoc: function(pokemonStats) {
		...
		if(pokemonStats.itemSet != null) {
			setList = new haxe_iterators_ArrayIterator(pokemonStats.itemSet);
		} else {
			setList = new haxe_ds__$StringMap_StringMapKeyIterator(this.database.descriptionsDataset.items.h);
		}
		var itemSlug = setList;
		while(itemSlug.hasNext()) {
			var itemSlug1 = itemSlug.next();
			itemRenderList.push({ "slug" : itemSlug1, "label" : this.database.descriptionsDataset.getItemName(itemSlug1), "selected" : itemSlug1 == pokemonStats.item ? "selected" : ""});
		}
	...
	,buildEditMoveRenderDoc: function(pokemonStats,slot) {
		...
		if(pokemonStats.itemSet != null && slot <= pokemonStats.moveSets.length - 1) {
			setList = new haxe_iterators_ArrayIterator(pokemonStats.moveSets[slot]);
		} else {
			setList = new haxe_ds__$StringMap_StringMapKeyIterator(this.database.movesDataset.moves.h);
		}
	...
	,testOne: function(slugs) {
		if(slugs.length > 0) {
			var slug = slugs.pop();
			haxe_Log.trace(slug,{ fileName : "src/visualizer/UI.hx", lineNumber : 836, className : "visualizer.UI", methodName : "testOne"});
			this.setSelectionBySlug(0,slug,false);
			this.syncSelectionListToCurrent();
			var _g = $bind(this,this.testOne);
			var slugs1 = slugs;
			var tmp = function() {
				_g(slugs1);
			};
...
visualizer_api_APIFacade.prototype = {
	...
	,getPokemonSets: function(callback) {
		...
		var cursorErrorHandler = function(xhr,textStatus,error) {
			_gthis.progressCallback = null;
			if(xhr.status == 410) {
				callbackResults();
			} else {
		...
		cursorPaginateSuccessHandler = function(jsonResult,textStatus,xhr) {
			try {
				var _g = 0;
				var _g1 = _gthis.parseMovesetPokemon(jsonResult);
				while(_g < _g1.length) {
					var stats = _g1[_g];
					++_g;
			...
			window.setTimeout(function() {
				_gthis.callAPI(visualizer_api_APIFacade.CONSUME_CURSOR_API_URL + token + "?limit=1000",cursorPaginateSuccessHandler,cursorErrorHandler,true);
			},1);
		};
		var cursorCreateSuccessHandler = function(jsonResult,textStatus,xhr) {
			token = jsonResult;
			window.setTimeout(function() {
				_gthis.callAPI(visualizer_api_APIFacade.CONSUME_CURSOR_API_URL + token + "?limit=1000",cursorPaginateSuccessHandler,cursorErrorHandler,true);
			},1);
	...
	,callAPI: function(url,done,error,post) {
		...
		if(this.callInProgress) {
			throw haxe_Exception.thrown("Call already in progress");
		}
	...
	,parsePokemonStats: function(jsonDoc,stats) {
		...
		if(Object.prototype.hasOwnProperty.call(jsonDoc,"item") && Reflect.field(jsonDoc,"item") != null) {
			...
			if(rawName != null) {
				stats.item = visualizer_api_APIFacade.slugify(rawName);
			}
		}
		stats.moves = [];
	...
	,parseMovesetPokemonStats: function(jsonDoc,stats) {
		...
		var _g = 0;
		while(_g < abilitiesDoc.length) {
			var doc = abilitiesDoc[_g];
			++_g;
		...
		var _g = 0;
		while(_g < itemsDoc.length) {
			var doc = itemsDoc[_g];
			++_g;
			var rawName = Reflect.field(doc,"name");
			if(rawName != null) {
				stats.itemSet.push(visualizer_api_APIFacade.slugify(rawName));
			}
		}
		var _g = 0;
		while(_g < movesDoc.length) {
			var slotDoc = movesDoc[_g];
			++_g;
			var slotMoves = [];
			var _g1 = 0;
			while(_g1 < slotDoc.length) {
				var moveDoc = slotDoc[_g1];
				++_g1;
				var moveSlug = visualizer_api_APIFacade.slugify(Reflect.field(moveDoc,"name"));
...
visualizer_dataset_DatasetLoadable.__isInterface__ = true;
...
visualizer_dataset_APIPokemonDataset.prototype = $extend(visualizer_dataset_Dataset.prototype,{
	...
	,getSlug: function(pokemonNum,movesetName) {
		...
		throw haxe_Exception.thrown(new visualizer_dataset_DatasetItemNotFoundError());
	}
	,loadPokemon: function(pokemonStatsList) {
		...
		while(_g < pokemonStatsList.length) {
			...
			this.stats.h[pokemonStats.slug] = pokemonStats;
		}
	...
	,loadFromStorage: function() {
		...
		if(versionStr == null) {
			throw haxe_Exception.thrown(new visualizer_dataset_StorageEmpty());
		}
		...
		while(_g < slugs.length) {
			...
			if(jsonStr == null) {
				haxe_Log.trace(jsonStr,{ fileName : "src/visualizer/dataset/APIPokemonDataset.hx", lineNumber : 112, className : "visualizer.dataset.APIPokemonDataset", methodName : "loadFromStorage", customParams : ["not found in storage. corrupted?"]});
				throw haxe_Exception.thrown(new visualizer_dataset_StorageEmpty());
			}
			var pokemonStats = new visualizer_datastruct_MovesetPokemonStats();
			pokemonStats.fromJsonObject(JSON.parse(jsonStr));
			this.stats.h[slug] = pokemonStats;
		}
		this.slugs = slugs;
		this.buildSpeciesIdSlugMap();
	}
	,saveToStorage: function() {
		var h = this.stats.h;
		var slug_h = h;
		var slug_keys = Object.keys(h);
		var slug_length = slug_keys.length;
		var slug_current = 0;
		while(slug_current < slug_length) {
			var slug = slug_keys[slug_current++];
			var pokemonStats = this.stats.h[slug];
			window.localStorage.setItem("" + visualizer_dataset_APIPokemonDataset.STORAGE_KEY + ":pokemon:" + slug,JSON.stringify(pokemonStats.toJsonObject()));
		}
	...
	,clearStorage: function() {
		var deleteKeys = [];
		var _g = 0;
		var _g1 = window.localStorage.length;
		while(_g < _g1) {
			var i = _g++;
		...
		var _g = 0;
		while(_g < deleteKeys.length) {
			var key = deleteKeys[_g];
			++_g;
	...
	,buildSpeciesIdSlugMap: function() {
		var h = this.stats.h;
		var pokemonStats_h = h;
		var pokemonStats_keys = Object.keys(h);
		var pokemonStats_length = pokemonStats_keys.length;
		var pokemonStats_current = 0;
		while(pokemonStats_current < pokemonStats_length) {
			var pokemonStats = pokemonStats_h[pokemonStats_keys[pokemonStats_current++]];
			if(!this.speciesIdToSlugMap.h.hasOwnProperty(pokemonStats.number)) {
				var this1 = this.speciesIdToSlugMap;
				var key = pokemonStats.number;
				var value = [];
				this1.h[key] = value;
			}
			this.speciesIdToSlugMap.h[pokemonStats.number].push(pokemonStats.slug);
		}
	}
...
visualizer_dataset_CurrentMatchDataset.prototype = {
	...
	,getPokemonStats: function(slug) {
		...
		throw haxe_Exception.thrown("Not found");
	}
...
visualizer_dataset_DescriptionsDataset.prototype = $extend(visualizer_dataset_Dataset.prototype,{
	...
	,loadDone: function(data) {
		...
		while(_g < _g1.length) {
			...
			this.abilities.h[slug] = { name : Reflect.field(doc,"name"), description : Reflect.field(doc,"description"), effectShort : Reflect.field(doc,"effect_short"), effectLong : Reflect.field(doc,"effect_long"), editorNote : Reflect.field(doc,"editor_note")};
		}
		this.types_efficacy = new haxe_ds_StringMap();
		var typesDoc = Reflect.field(data,"types_efficacy");
		var _g = 0;
		var _g1 = Reflect.fields(typesDoc);
		while(_g < _g1.length) {
			var firstType = _g1[_g];
			++_g;
			...
			while(_g2 < _g3.length) {
				var secondType = _g3[_g2];
				++_g2;
				var efficacy = Reflect.field(secondTypesDoc,secondType);
				this.types_efficacy.h["" + firstType + "*" + secondType] = efficacy;
			}
		}
		this.items = new haxe_ds_StringMap();
		var itemsDoc = Reflect.field(data,"items");
		var _g = 0;
		var _g1 = Reflect.fields(itemsDoc);
		while(_g < _g1.length) {
			var slug = _g1[_g];
			++_g;
			var doc = Reflect.field(itemsDoc,slug);
			this.items.h[slug] = { name : Reflect.field(doc,"name"), description : Reflect.field(doc,"description"), effectShort : Reflect.field(doc,"effect_short"), effectLong : Reflect.field(doc,"effect_long"), flingEffectShort : Reflect.field(doc,"fling_effect_short"), flingEffectLong : Reflect.field(doc,"fling_effect_long"), flingPower : Reflect.field(doc,"fling_power"), naturalGiftPower : Reflect.field(doc,"natural_gift_power"), naturalGiftType : Reflect.field(doc,"natural_gift_type")};
		}
		this.itemRenames = new haxe_ds_StringMap();
		var itemRenamesDoc = Reflect.field(data,"item_renames");
		var _g = 0;
		var _g1 = Reflect.fields(itemRenamesDoc);
		while(_g < _g1.length) {
			var oldName = _g1[_g];
			++_g;
			var newName = Reflect.field(itemRenamesDoc,oldName);
			this.itemRenames.h[oldName] = newName;
		}
		this.buildDashlessSlugMap();
		visualizer_dataset_Dataset.prototype.loadDone.call(this,data);
	}
	,getAbility: function(slug) {
		if(!Object.prototype.hasOwnProperty.call(this.abilities.h,slug)) {
			slug = this.dashlessSlugMap.h[slug];
		}
		return this.abilities.h[slug];
	}
	,getAbilityName: function(slug) {
		return this.getAbility(slug).name;
	}
	,getItem: function(slug) {
		if(Object.prototype.hasOwnProperty.call(this.items.h,slug)) {
			return this.items.h[slug];
		}
		var candidateSlugs = [];
		if(Object.prototype.hasOwnProperty.call(this.itemRenames.h,slug)) {
			candidateSlugs.push(this.itemRenames.h[slug]);
		}
		if(Object.prototype.hasOwnProperty.call(this.dashlessSlugMap.h,slug)) {
			var dashless = this.dashlessSlugMap.h[slug];
			candidateSlugs.push(dashless);
			if(Object.prototype.hasOwnProperty.call(this.itemRenames.h,dashless)) {
				candidateSlugs.push(this.itemRenames.h[dashless]);
			}
		}
		var _g = 0;
		while(_g < candidateSlugs.length) {
			...
			++_g;
			if(Object.prototype.hasOwnProperty.call(this.items.h,candidate)) {
				return this.items.h[candidate];
			}
		}
		return null;
	}
	...
	,getTypeEfficacy: function(user,foe,foeSecondary) {
		var efficacy = this.types_efficacy.h["" + user + "*" + foe];
		if(foeSecondary == null) {
			return efficacy;
		}
		var secondaryEfficacy = this.types_efficacy.h["" + user + "*" + foeSecondary];
		var pair_0 = efficacy;
	...
	,buildDashlessSlugMap: function() {
		...
		while(key_current < key_length) {
			...
			var key1 = visualizer_api_APIFacade.slugify(key,true);
			this1.h[key1] = key;
		}
		var h = this.abilities.h;
		var key_h = h;
		var key_keys = Object.keys(h);
		var key_length = key_keys.length;
		var key_current = 0;
...
visualizer_dataset_MovesDataset.prototype = $extend(visualizer_dataset_Dataset.prototype,{
	...
	,loadDone: function(data) {
		...
		while(_g < _g1.length) {
			...
			this.moves.h[slug] = moveStats;
		}
		visualizer_dataset_Dataset.prototype.loadDone.call(this,data);
	}
	,getMoveStats: function(slug,pokemonStat) {
		var moveStat = this.moves.h[slug].copy();
...
visualizer_dataset_PokemonDataset.prototype = $extend(visualizer_dataset_Dataset.prototype,{
	...
	,getPokemonStats: function(slug) {
		var this1 = this.get_stats();
		if(!Object.prototype.hasOwnProperty.call(this1.h,slug)) {
			throw haxe_Exception.thrown(new visualizer_dataset_DatasetItemNotFoundError());
		}
		var pokemonStat = new visualizer_datastruct_PokemonStats();
		pokemonStat.slug = slug;
		pokemonStat.update(this.get_stats().h[slug]);
		return pokemonStat;
	}
...
visualizer_datastruct_Copyable.__isInterface__ = true;
...
visualizer_model_PokemonDatabase.prototype = {
	...
	,getPokemonStats: function(slug) {
		var pokemonStats = new visualizer_datastruct_VisualizerPokemonStats();
		if(Object.prototype.hasOwnProperty.call(this.customStats.h,slug)) {
			pokemonStats.update(this.customStats.h[slug]);
			return pokemonStats;
			...
			if(stats != null) {
				...
				throw haxe_Exception.thrown(new visualizer_model_StatsNotFoundError());
			}
		} else {
			try {
				var stats = this.pokemonDataset.getPokemonStats(slug);
				pokemonStats.update(stats);
				return pokemonStats;
			} catch( _g ) {
				if(((haxe_Exception.caught(_g).unwrap()) instanceof visualizer_dataset_DatasetItemNotFoundError)) {
					throw haxe_Exception.thrown(new visualizer_model_StatsNotFoundError());
				} else {
					throw _g;
				}
	...
	,isCustomized: function(slug) {
		return Object.prototype.hasOwnProperty.call(this.customStats.h,slug);
	}
...
visualizer_api_APIFacade.POKEMON_SETS_API_URL = "https://twitchplayspokemon.tv/api/pokemon_sets?id&limit=1000";
